...
import Model
from utils import (find_min_and_max, subband2patch, subband2patch_padding,
...
def enc_lossless(args):
    ...
    # model_path = args.model_path + '/' + 'model_epoch076' + '.pth'
    ...
    with torch.no_grad():
        ...
        enc_LL = subband2patch(enc_LL, code_block_size, code_block_size, code_block_size)
        paddings = (2, 2, 2, 2)
        enc_LL = F.pad(enc_LL, paddings, "constant")
        for h_i in range(code_block_size):
            for w_i in range(code_block_size):
                cur_ct = copy.deepcopy(enc_LL[:, :, h_i:h_i + 5, w_i:w_i + 5])
                cur_ct[:, :, 5 // 2 + 1:5, :] = 0.
                cur_ct[:, :, 5 // 2, 5 // 2:5] = 0.
                prob = models_dict['entropy_LL'](cur_ct, yuv_low_bound[0], yuv_high_bound[0])

                prob = prob.cpu().data.numpy()
                index = enc_LL[:, 0, h_i + 2, w_i + 2].cpu().data.numpy().astype(np.int)
        ...
        for i in range(trans_steps):
            ...
            enc_oth = subband2patch(enc_oth, code_block_size, code_block_size, code_block_size)
            paddings = (2, 2, 2, 2)
            enc_oth = F.pad(enc_oth, paddings, "constant")

            paddings = (0, padding_sub_w[j], 0, padding_sub_h[j])
            # context = F.pad(LL, paddings, "reflect")
            if padding_sub_w[j] >= LL.shape[3] or padding_sub_h[j] >= LL.shape[2]:
                context = F.pad(LL, paddings, "constant")
            else:
                context = F.pad(LL, paddings, "reflect")

            paddings = (2, 2, 2, 2)
            context = F.pad(context, paddings, "reflect")
            context = subband2patch_padding(context, code_block_size+4, code_block_size+4, code_block_size, 2)
            for h_i in range(code_block_size):
                for w_i in range(code_block_size):
                    cur_ct = copy.deepcopy(enc_oth[:, :, h_i:h_i + 5, w_i:w_i + 5])
                    cur_ct[:, :, 5 // 2 + 1:5, :] = 0.
                    cur_ct[:, :, 5 // 2, 5 // 2:5] = 0.
                    cur_context = context[:, :, h_i:h_i + 5, w_i:w_i + 5]
                    prob = models_dict['entropy_HL'](cur_ct, cur_context,
                                                              yuv_low_bound[3 * j + 1], yuv_high_bound[3 * j + 1], j)

                    prob = prob.cpu().data.numpy()
                    index = enc_oth[:, 0, h_i + 2, w_i + 2].cpu().data.numpy().astype(np.int)
                    # index = index - lower_bound
            ...
            temp_context = torch.cat((LL, HL_list[j]), dim=1)
            ...
            temp_context = torch.cat((LL, HL_list[j], LH_list[j]), dim=1)
            if padding_sub_w[j] >= temp_context.shape[3] or padding_sub_h[j] >= temp_context.shape[2]:
                context = F.pad(temp_context, paddings, "constant")
            else:
                context = F.pad(temp_context, paddings, "reflect")
        ...
        with open(f'/data/yuyue/Test_iwave_qint8/a01_classBtest/{model_lambdas[args.model_qp]}_new_BPP.txt', 'a') as f:
            f.write('\n'+str(filesize))

        with open(f'/data/yuyue/Test_iwave_qint8/a01_classBtest/{model_lambdas[args.model_qp]}_new_PSNR.txt', 'a') as f:
            f.write('\n'+str(psnr))
