...
from utils import (find_min_and_max, subband2patch, patch2subband, subbands2patch, patch2subbands, get_entropy_params,
                       model_lambdas, qp_shifts, rgb2yuv, yuv2rgb, get_prob, arithmetic_coding, save_params, write_header)
import sys
...
def enc_lossy(args):
    ...
    bin_path = os.path.join(args.bin_dir, bin_name + '_v3.bin')
    ...
    with torch.no_grad():
        ...
        input0 = input0.data.numpy().astype(np.uint8)
        ...
        save_coefficient_path = '/data/kangbw/models/hardware_project/output/enc_coefficient/v3/'
        ...
        # save all the tensors and then empty the cache
        save_params_path = '/data/kangbw/models/hardware_project/output/enc_params/v3/'
        ...
        # arithmetic coding, gmm with float params and probs 
        gmm = Distribution_for_entropy2()
        coded_coe_num = 0
        code_block_bytes_list = []
        # compress LL

        params = torch.load(save_params_path+f'params_LL.pt')
        prob = get_prob(params, yuv_low_bound, yuv_high_bound, gmm, freqs_resolution, 0)
        coded_coe_num = arithmetic_coding(bin_path, code_block_bin_path, LL, prob, shift_min, shift_max, min_v, patch_num, y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, code_block_bytes_list, code_block_size, coded_coe_num, 0)
        print('LL encoded...')
        # for i in range(trans_steps):
        #     j = trans_steps - 1 - i
        #     # compress HL
        #     params = torch.load(save_params_path+f'params_HL_list_{j}.pt')
        #     prob = get_prob(params, yuv_low_bound, yuv_high_bound, gmm, freqs_resolution, 3*j+1)
        #     coded_coe_num = arithmetic_coding(bin_path, code_block_bin_path, HL_list[i], prob, shift_min, shift_max, min_v, patch_num, y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, code_block_bytes_list, code_block_size, coded_coe_num, 3*j+1)
        #     print('HL' + str(j) + ' encoded...')
            
        #     # compress LH
        #     params = torch.load(save_params_path+f'params_LH_list_{j}.pt')
        #     prob = get_prob(params, yuv_low_bound, yuv_high_bound, gmm, freqs_resolution, 3*j+2)
        #     coded_coe_num = arithmetic_coding(bin_path, code_block_bin_path, LH_list[i], prob, shift_min, shift_max, min_v, patch_num, y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, code_block_bytes_list, code_block_size, coded_coe_num, 3*j+2)
        #     print('LH' + str(j) + ' encoded...')
            
        #     # compress HH
        #     params = torch.load(save_params_path+f'params_HH_list_{j}.pt')
        #     prob = get_prob(params, yuv_low_bound, yuv_high_bound, gmm, freqs_resolution, 3*j+3)
        #     coded_coe_num = arithmetic_coding(bin_path, code_block_bin_path, HH_list[i], prob, shift_min, shift_max, min_v, patch_num, y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, code_block_bytes_list, code_block_size, coded_coe_num, 3*j+3)
        #     print('HH' + str(j) + ' encoded...')
    
            # reconstruct the image using the inverse transform model which runs on GPU/FPGA...
            # Note that the inverse trans here is just to get the reconstructed image and validate the performance, which will not be employed on the Encoder.
            # The inverse_trans which used in order to get the context is in the entropy model, which us called in the forward
        # write picture header to the bin file
        write_header(bin_path, args.isLossless, args.model_qp, args.qp_shift, args.isPostGAN, height, width, min_v, max_v, code_block_bytes_list)
        print('coded_coe_num: ', coded_coe_num)
        ...
        filesize = os.path.getsize(bin_path) * 8 / (height * width)
        print('BPP: ', filesize)
