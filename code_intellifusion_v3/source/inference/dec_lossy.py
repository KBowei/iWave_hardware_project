...
import Model
from utils import (read_header, model_lambdas, qp_shifts, yuv2rgb, subband2patch_discard, subbands2patch, patch2subbands, autoregressive_dec)
    ...
    for c in range(1):
        ...
        for i in range(trans_step):
            ...
            result_image[:, :, h_list[0]:h_list[i + 1],
                            w_list[i + 1]:w_list[i + 2]] = HL_list[c][j]
            result_image[:, :, h_list[i + 1]:h_list[i + 2],
                            w_list[0]:w_list[i + 1]] = LH_list[c][j]
            result_image[:, :, h_list[i + 1]:h_list[i + 2],
                            w_list[i + 1]:w_list[i + 2]] = HH_list[c][j]
...
def dec_lossy(args, bin_path, freqs_resolution, logfile):
    ...
    with torch.no_grad():
        ...
        freqs = ac.SimpleFrequencyTable(np.ones([2], dtype=int))
        if isPostGAN:
        ...
        coded_coe_num = 0
        saved_path = '/data/kangbw/models/hardware_project/output/decode/v3/'
        ...
        patch_idx = 0
        # decompress LL  
        LL, coded_coe_num, patch_idx = autoregressive_dec(bin_path, LL, models_dict['coding_LL'], yuv_low_bound, yuv_high_bound, shift_max, shift_min, min_v, patch_num, code_block_bytes_list,  
                                               y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, 
                                               gmm, freqs_resolution, code_block_size, coded_coe_num, 0, patch_idx, content_bits_start_position)
        print('LL decoded')
        ...
        #     j = trans_steps - 1 - i
        #     # decompress HL
        #     HL, coded_coe_num, patch_idx = autoregressive_dec(bin_path, torch.cat((HL_list[i], context), dim=1), models_dict['coding_HL'], yuv_low_bound, yuv_high_bound, shift_max, shift_min, min_v, patch_num, code_block_bytes_list,  
        #                                            y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, 
        #                                            gmm, freqs_resolution, code_block_size, coded_coe_num, 3*j+1, patch_idx, content_bits_start_position)
        #     HL_list[i] = HL
        ...
        #     # decompress LH
        #     LH, coded_coe_num, patch_idx = autoregressive_dec(bin_path, torch.cat((LH_list[i], HL, context), dim=1), models_dict['coding_LH'], yuv_low_bound, yuv_high_bound, shift_max, shift_min, min_v, patch_num, code_block_bytes_list,  
        #                                            y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, 
        #                                            gmm, freqs_resolution, code_block_size, coded_coe_num, 3*j+2, patch_idx, content_bits_start_position)
        #     LH_list[i] = LH
        ...
        #     # decompress HH
        #     HH, coded_coe_num, patch_idx = autoregressive_dec(bin_path, torch.cat((HH_list[i], LH, HL, context), dim=1), models_dict['coding_HH'], yuv_low_bound, yuv_high_bound, shift_max, shift_min, min_v, patch_num, code_block_bytes_list, 
        #                                            y_subband_h, y_subband_w, uv_subband_h, uv_subband_w, 
        #                                            gmm, freqs_resolution, code_block_size, coded_coe_num, 3*j+3, patch_idx, content_bits_start_position)
        #     HH_list[i] = HH
